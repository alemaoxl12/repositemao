  ...
  async welcome({ m, participants }) {
    ...
    for (let user of participants) {
      ...
      try {
        ...
        let text = (chat.sWelcome || this.welcome || conn.welcome || 'BEM VINDO, @user!').replace('@user', '@' + user.split('@')[0]).replace('@subject', this.getName(m.key.remoteJid))
